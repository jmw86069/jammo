...
#'
#' Fix matrix dimension ratio for image interpolation
#'
...
#'    main="useRaster=TRUE\ninterpolate=TRUE\nfixRasterRatio=FALSE");
#' imageByColors(m2, useRaster=TRUE, interpolate=TRUE, fixRasterRatio=TRUE,
#'    main=paste0("fix_matrix_ratio(m)\nuseRaster=interpolate=TRUE",
#'    "\nfixRasterRatio=TRUE"));
...
#' g1 <- seq(from=100, to=10000, by=1000);
#' n <- 5;
#' k <- sort(g1+rep(seq_len(n), each=length(g1)))
#' x <- 15;
#' testHeatdata[k, ] <- x;
#' for (i in seq(from=125, to=235, by=3)) {
#'    ix <- round(sin(deg2rad(i))*5+5);
#'    iy <- round(-cos(deg2rad(i))*5500 + 3500);
#'    testHeatdata[iy:(iy + n), ix] <- x;
#' }
#' g2 <- 3011+c(1:12*90);
#' k2 <- g2+rep(seq_len(n), each=length(g2))
#' testHeatdata[k2, c(3,7)] <- x;
#' testHeatdata <- testHeatdata[10000:1, ];
#' col <- getColorRamp("RdBu_r", n=15, lens=1, trimRamp=c(5,1));
#' graphics::par("mfrow"=c(1, 2));
#' image.default(z=t(testHeatdata), col=col, useRaster=TRUE, oldstyle=FALSE,
